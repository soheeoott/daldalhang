...
public class FranchiseController {
	...
	public ModelAndView fcinsert(ModelAndView mv, FranchiseVO fvo) throws Exception{
		...
		String path = "D:\\workspace\\daldal\\src\\main\\webapp\\resources\\img_logo\\"; //폴더 경로
		...
		if(fvo.getFruploadfilef() != null) {
			...
			if(!fruploadfilef.isEmpty()) {
				...
				// 실제 저장 경로 생성하고 저장
				file1="D:/workspace/daldal/src/main/webapp/resources/img_logo/"+fruploadfilef.getOriginalFilename(); // 드라이브에 저장되는 실제 경로
		...
		if (frservice.insert(fvo) > 0) {
			// 성공
			mv.setViewName("redirect:franchiseList");
		} else {
			// 실패
			mv.setViewName("home");
		}
	...
	public ModelAndView fdetail(ModelAndView mv, FranchiseVO fvo, HttpSession session) throws Exception {
		
		List<MenuVO> menulist = muservice.menu();
		session.setAttribute("menulist", menulist);
		
		fvo = frservice.fdetail(fvo);
		mv.addObject("fdetail", fvo);
		mv.setViewName("franchise/franchiseDetail");
		return mv;
	}
	...
	@RequestMapping(value="/franchiseMainList")
